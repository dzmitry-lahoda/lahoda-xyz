...
{
    public class UdpPingClient : IDisposable
    {
        ...
        bool m_Disposed; // To detect redundant calls
        ...
        public UdpPingClient(NetworkEndPoint serverEndpoint)
        {
        ...
        {
            if (m_RemoteServerHasShutDown)
            {
                Debug.LogWarning($"{nameof(ShutdownRemoteServer)} called when server has already shut down.");
                return;
            }

            if (m_Disposed)
            {
                Debug.LogWarning($"{nameof(ShutdownRemoteServer)} called when {nameof(UdpPingClient)} has already been disposed.");
                return;
            }
        ...
        public void RunPingGenerator()
        {
            if (m_RemoteServerHasShutDown)
            {
                Debug.LogWarning($"{nameof(RunPingGenerator)} called when server has already shut down.");
                return;
            }

            if (m_Disposed)
            {
                Debug.LogWarning($"{nameof(RunPingGenerator)} called when {nameof(UdpPingClient)} has already been disposed.");
                return;
            }

            // Don't run a ping job unless we're done with existing work
        ...
        struct PingJob : IJob
        {
            ...
            [WriteOnly]
            public NativeArray<ushort> lastPing;
